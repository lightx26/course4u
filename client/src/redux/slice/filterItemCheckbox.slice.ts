...
export const filterItemSlice = createSlice({
  ...
  reducers: {
    ...
    toggleFilterItem: (
      state,
      action: PayloadAction<FilterItemSliceType & { choiceId: string }>
    ) => {
      const componentIndex = state.findIndex(
        (item) => item.FilterComponentId === action.payload.FilterComponentId
      );

      if (componentIndex !== -1) {
        const choiceIndex = state[componentIndex].listChoice.findIndex(
    ...
    handleViewAllByDetailCousePage: (
      state,
      action: PayloadAction<FilterItemSliceType>
    ) => {
      state.length = 0;
      state.push(action.payload);
    },
