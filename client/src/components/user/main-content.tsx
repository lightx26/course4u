...
import ListCourseCardComponent from "../course/grid-view/list-course-card.tsx";
import PaginationSection from "../shared/pagination-section.tsx";

import EmptyPage from "../../screens/empty-page.tsx";
...
import {
    ...
    updatePage,
    updateSort,
} from "../../redux/slice/course.slice.ts";
import {
    Select,
    SelectContent,
    SelectItem,
...
export default function MainContent() {
    ...
    const courseState: CourseStateType = useSelector(
    ...
    const length = Array.from(
        new Set(
            selectedItems.flatMap((item) =>
                item.listChoice.map((choice) => choice.name)
            )
        )
    ).length;
        ...
        <div className='flex flex-col gap-5 w-[80%] p-2 grow'>
            <div className='flex flex-wrap gap-2'>
                {selectedItems.map((item) =>
                    item.listChoice.map((choice) => (
                        <div
                            ...
                            key={choice.id}
                            className='px-2 py-1 text-[12px] cursor-pointer bg-gray-100 rounded-md'
                        >
                            <p className='flex items-center'>
                                {choice.name}{" "}
                                <XIcon className='w-4 ml-1 hover:text-violet-600' />
                            </p>
                        </div>
                    ))
                )}
                {length > 1 && (
                    ...
                    >
                        <p className='flex items-center'>
                            Delete All <XIcon className='w-4 ml-1 text-white' />
                        </p>
                    </div>
                )}
            </div>
            {!isLoading && courseState.content.length == 0 ? (
                <EmptyPage
                    content={
                        "No courses found. Try changing your search terms, adjusting your filters, or exploring different categories to find what you're looking for."
                    }
                />
            ) : (
                <>
                    ...
                    <ListCourseCardComponent
                        ListCourse={courseState.content}
                        isLoading={isLoading}
