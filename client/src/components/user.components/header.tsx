...
const HeaderHomepage: React.FC = () => {
  ...
  const handleSearch = () => {
    ...
    if (userData.role.toUpperCase() == 'USER' && window.location.pathname != basePath) {
      goToHomePage();
    }
    else if (userData.role.toUpperCase() == 'ADMIN' && window.location.pathname != (basePath + '/admin/courses').replace('//', '/')) {
  ...
  return (
    ...
    >
      <div className="container flex items-center justify-between max-w-screen-xl mx-auto">
        ...
        {userData.role != "USER" &&
          <div className="px-4 text-2xl font-black text-purple">{userData.role.toUpperCase()}</div>
        }
        {userData.role.toUpperCase() != 'ACCOUNTANT' && <div className='relative flex-grow mx-6'>
        ...
        <div className='flex items-center space-x-4'>
          <div className='flex items-center justify-center gap-5'>
            {userData.role.toUpperCase() != 'ACCOUNTANT'
              && (userData.role.toUpperCase() == 'USER' ? <button
            ...
            <div className='flex items-center gap-6'>
              {userData.role == "USER" && <div title="Leader Board">
          ...
          <div className="relative w-[40px] h-[40px] rounded-[50%] group">
            ...
            <div className="absolute hidden group-hover:block right-2 top-10 z-[1000]">
              {userData.role.toUpperCase() == 'USER' ? <UserCard /> :
                userData.role.toUpperCase() == 'ADMIN' ? <AdminCard /> : <></>}
            </div>
          </div>
        </div>
      </div>
