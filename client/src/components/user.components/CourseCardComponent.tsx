...
export default function CourseCardComponent({ course }: PropsType) {
    return (
        <a href='' className='w-full overflow-hidden text-sm leading-normal transition-shadow bg-white rounded-md shadow cursor-pointer duration-250 hover:shadow-lg grow'>
            <div>
                ...
                <div className='flex flex-col gap-2 px-2 pt-2 text-left'>
                    <h3 className='text-base font-medium text-left text line-clamp-2 overflow-ellipsis h-14 w-'>{course.name}</h3>
                    {/*Assignee role and status are optional*/}
                    {
                        course.assignee &&
                        <div className='flex items-center gap-2'>
                            <img className='object-cover rounded-full w-11 h-11' src={course.assignee?.avatarUrl} alt={course.assignee?.name} />
                            <div>
                                <p className='text-base font-medium'>{course.assignee?.name}</p>
                                {course.assignee?.role && <p className='text-xs text-gray-500'>{course.assignee?.role}</p>}
                            </div>
                    ...
                    <div className='flex0'>
                        <p>{course.platform}</p>
                        {course.level && <p className='px-2 py-2 bg-blue-400'>{course.level}</p>}
                    </div>
                    <p>{course.period.startDay.toDateString()} - {course.period.endDay.toDateString()}</p>
                        ...
                        <div className='flex gap-1'>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                ...
                                <path
                                    ...
                                    stroke="#564FFD" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
