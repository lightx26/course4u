...
import { handleEditProfile } from "../../../apiService/user.service.ts"
...
export default function UserProfileForm() {
    ...
    useEffect(() => {
        ...
        setAvatar({ ...avatar, imageUrl: userInfor.avatarUrl.startsWith("http") || userInfor.avatarUrl.startsWith("data:image") ? userInfor.avatarUrl : `${import.meta.env.VITE_BACKEND_URL}${userInfor.avatarUrl}` })
    }, [userInfor])
    ...
    async function onSubmit(values: z.infer<typeof editUserSchema>) {
        ...
        if (values.dateOfBirth) {
            ...
            formData.append('dateOfBirth', dateWithTime.toISOString().split('T')[0]);
        }
    ...
    const onDeleteImage = () => {
        setAvatar({ ...avatar, imageUrl: undefined, croppedImageUrl: undefined });
                        ...
                        <div className="flex w-[30%]  grow flex-col gap-4">
                            <div>
                                <div className='relative flex justify-center gap-4 text-base'>
                                    {avatar.imageUrl && avatar.imageUrl !== '' && avatar.imageUrl != `${import.meta.env.VITE_BACKEND_URL}` ? (
                                        <CropThumbnail
                                                    ...
                                                    <FormControl>
                                                        ...
                                                        >
                                                            ...
                                                            <h4 className='text-sm p-3 font-semibold group-hover:text-[#4fadff] text-center'>
                                                                {isDragActive ? 'Drop it here...' : 'Choose a file or drag it here'}
                                                            </h4>
                                                        </div>
                                                    </FormControl>
                                                </FormItem>
