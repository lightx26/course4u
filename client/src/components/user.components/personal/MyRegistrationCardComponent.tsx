...
export default function MyRegistrationComponents({ registration }: PropsType) {
    const startDate = typeof registration.startDate === "string" ? new Date(registration.startDate) : registration.startDate;
    const endDate = registration.endDate ? (typeof registration.endDate === "string" ? new Date(registration.endDate) : registration.endDate) : null;
        ...
        <div className='pb-5 mb-3 w-full overflow-hidden text-sm leading-normal transition-shadow bg-white rounded-md shadow cursor-pointer duration-250'>
            <div>
                ...
                <div className='flex flex-col gap-2 px-2 pt-2 text-left'>
                    <h2 className='text-lg font-normal'>{registration.courseName}</h2>
                    <p
...
type Status = "DRAFT" | "DISCARDED" | "SUBMITTED" | "DECLINED" | "DONE" | "APPROVED" | "VERIFYING" | "VERIFIED" | "CLOSED" | "DOCUMENT_DECLINED";
