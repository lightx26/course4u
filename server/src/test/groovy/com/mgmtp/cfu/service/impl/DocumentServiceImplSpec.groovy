...
class DocumentServiceImplSpec extends Specification {
    ...
    def "should approve document"() {
        given:
        def documentId = 1L
        def document = new Document(id: documentId, status: DocumentStatus.PENDING)
        def documentDTO = new DocumentDTO(id: documentId)
        def mapper = Mock(DTOMapper) {
            toDTO(_ as Document) >> documentDTO
        }

        documentRepository.findById(documentId) >> Optional.of(document)
        documentRepository.save(document) >> document
        documentDtoMapperFactory.getDTOMapper(DocumentDTO.class) >> Optional.of(mapper)

        when:
        def result = documentServiceImpl.approveDocument(documentId)
    ...
    def "should decline document"() {
        ...
        when:
        def result = documentServiceImpl.declineDocument(documentId)
    ...
    def "should throw exception if document not found for approval"() {
        given:
        def documentId = 1L
        ...
        when:
        documentServiceImpl.approveDocument(documentId)

        then:
        def e = thrown(BadRequestRuntimeException)
        e.message == "Document not found."
    }

    def "should throw exception if document not found for decline"() {
        given:
        def documentId = 1L
        ...
        when:
        documentServiceImpl.declineDocument(documentId)
